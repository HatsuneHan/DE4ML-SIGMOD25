...
def train_saga(weight, bias, loader, val_loader, *,
    ...
    a_table.to(weight.device)
    shuttle.to(weight.device)
    ...
    try:
        while True:
            ...
            for bool_X, y, idx in iterator:
                ...
                # a_table[:, :num_keep].index_copy_(0, idx.cpu(), residual[:, :num_keep].cpu())
            ...
            if t % early_stop_freq == early_stop_freq - 1:
                ...
                still_opt_outer.to(weight.device)
    ...
    del residual, a_table, shuttle, a_prev, w_saga, b_saga, deltas, deltas_inds
    del w_grad_avg, b_grad_avg, train_losses, total_train_losses
    del new_fin_mask
    ch.cuda.empty_cache()
