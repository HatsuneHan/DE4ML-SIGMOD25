...
def process_result(results, df_info, target_name):
    '''
    ...
    for k in results.keys():
        ...
        for i in range(len(results[k])):
            ...
            if len(df_info.numerical_cols) > 0:
            ...
            # final_df = final_df.join([origin_input_df])
            ...
            if not results[k][i]['adv_example_df'] is None:
                ...
                origin_ae_columns = [
                    f"{col}" for col in scaled_ae_df.columns]
                ...
                final_df = final_df.join([origin_ae_df])
            ...
            final_df[target_name] = results[k][i]['ground_truth']
