...
def get_mim_instance(wrapped_models, norm, mim_params):
    '''
    '''
    adv_instance = {}
    ...
    for k in wrapped_models.keys():
        ...
        adv_instance[k] = MomentumIterativeMethod(estimator=wrapped_models[k], **mim_params)

    return adv_instance


def generate_mim_result(
        df_info: DfInfo,
        models,
        num_instances,
        batch_size,
    ...
    adv_instance = get_mim_instance(wrapped_models, norm=norm, mim_params=mim_params)

    # Initialise the result dictionary.(It will be the return value.)
    results = {}

    if isinstance(num_instances, int) and num_instances % mim_params['batch_size'] == 0:
        ...
        X_test_num = len(X_test) - (len(X_test)%mim_params['batch_size'])
        X_test_re=X_test[0:X_test_num]
        y_test_num = len(y_test) - (len(y_test)%mim_params['batch_size'])
