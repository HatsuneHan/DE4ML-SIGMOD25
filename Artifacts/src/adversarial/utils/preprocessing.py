...
def inverse_dummy(dummy_df, cat_to_ohe_cat):
    ...
    for k in cat_to_ohe_cat.keys():
        ...
        if k == "Person":
            not_dummy_df[k] = not_dummy_df[k].apply(lambda x: x if x.startswith("Person_under_18") else x.replace(f'{k}_', ""))
        else:
...
def get_cat_ohe_info(dummy_df, categorical_cols, target_name):
    ...
    for c_col in categorical_cols:
        if c_col != target_name:
            if c_col == "Person":
                cat_to_ohe_cat[c_col] = [ ohe_col for ohe_col in dummy_df.columns if ohe_col.startswith(c_col+'_') and ohe_col != target_name and not ohe_col.startswith("Person_under_18")]
            else:
                cat_to_ohe_cat[c_col] = [ ohe_col for ohe_col in dummy_df.columns if ohe_col.startswith(c_col+'_') and ohe_col != target_name]
